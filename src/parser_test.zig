...
test "Test Integer expression statement" {
    ...
    const expected_integers = [_][]const u8{ "5", "10", "4242" };

    for (program.statements.items, expected_integers) |st, expected| {
        const expr_st = st.expr_statement;
        ...
        const int_value = try std.fmt.parseInt(u64, expected, 10);
